...
async function start(){
  ...
  // if import-result element exists (import UI), show a short notice
  const importEl = el('import-result');
  if(importEl){
    if(removed>0) importEl.textContent = `Đã loại ${removed} câu trùng/giống trước khi bắt đầu.`; else importEl.textContent = '';
  }
  // read mode/options
  ...
  const optAiGrade = !!el('opt-ai-grade') && el('opt-ai-grade').checked;
  // attach options to global for use in render/submit
  window.__quizOptions = { optShuffle, optLimit, optTextCheck, optAiGrade };
...
function renderQuestion(){
  ...
  if(q.type === 'mcq'){
    ...
    const checkAnswer = async ()=>{
      ...
      if(aiGrade){
        // call AI grader endpoint
        try{
          const res = await fetch('/ai/grade', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({question:q.question, answer:q.shortAnswer||'', response:user, text_check:textCheck}) });
          const j = await res.json(); score = j.score; verdict = j.verdict || j.feedback || '';
        }catch(e){ verdict = 'AI grader không phản hồi: '+e.message; }
      } else {
        // simple text-based check: token overlap
